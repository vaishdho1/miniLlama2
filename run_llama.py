...
class LlamaDataset(Dataset):
	def __init__(self, dataset, args, eos=False,flag = True):
	...
	def collate_fn(self, all_data):
		...
		if self.flag:
		    ...
		    batched_data = {'token_ids':token_ids,'sents':sents}
	   

		return batched_data

def print_wrong_sentences(pred,true,sents,file):
    with open(file,'w') as f:
        for i in range(len(pred)):
            if pred[i] != true[i]:
                f.write(f"{pred[i]}| {true[i]} | {sents[i]}");
                f.write("\n")
...
#Creating data for next word prediction of the form (sentence,token)
def create_word_data(filename,tokenizer: Tokenizer, eos: bool = True, prompt_suffix: Optional[str]=None):
    data = []
    tot = 0
    with open(filename,'r') as fp:
        for line in fp:
...
def model_loss(dataloader, model,tokenizer, device):
    model.eval() # switch to eval model, will turn off randomness like dropout
    numBatches = 0;
    loss = 0
    with torch.no_grad():
        for step, batch in enumerate(tqdm(dataloader, desc=f'eval', disable=TQDM_DISABLE)):
    	    b_ids, b_sents = batch['token_ids'], batch['sents']
    	    input_ids = b_ids[:,:-1]
    	    target_ids = b_ids[:,1:]
    	    input_ids = input_ids.to(device)
    	    target_ids = target_ids.to(device)
    	    logits,_ = model(input_ids,target_ids)
    	    loss += F.cross_entropy(logits.view(-1,logits.size(-1)),target_ids.view(-1),ignore_index=tokenizer.pad_id,reduction='mean')
    	    numBatches += 1
        loss = loss/numBatches
    return loss
    
    
def save_model(model, optimizer, args,config, filepath,model_args=None):
	save_info = {
...
def pretrain(args):
    ...
    train_data = create_word_data(args.train,tokenizer)
    dev_data = create_word_data(args.dev,tokenizer)
    train_dataset = LlamaDataset(train_data,args,flag=False)
    dev_dataset = LlamaDataset(dev_data,args,flag = False)
    ...
    model,model_args = load_pretrained(config.pretrained_model_path)
    ...
    print(f'Total trainable paramters : {trainable_params}')
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc=f'train-{epoch}', disable=TQDM_DISABLE)):
			...
			#Compute mean loss across all valid tokens across a group of batch specified
            loss = F.cross_entropy(logits.view(-1,logits.size(-1)),target_ids.view(-1),ignore_index=tokenizer.pad_id,reduction='mean')
        ...
        dev_loss = model_loss(dev_dataloader, model,tokenizer, device)
       
        if dev_loss < best_dev_loss:
            best_dev_loss = dev_loss
            save_model(model, optimizer, args, model_args, config, args.filepath)
        print(f"epoch {epoch}: train loss :: {train_loss :.3f}, train dev_loss :: {dev_loss :.3f}")
...
def train(args):
	...
	# If I want to use lora to finetune
	if args.option == 'lora':
	    target = args.modules.strip().split(',')
	    model = addLoraWrapper(model,target,args.rank,args.alpha,args.lora_dropout)
	...
	lr = args.lr
	## specify the optimizer added regularization
	optimizer = AdamW(
    filter(lambda p: p.requires_grad, model.parameters()),weight_decay = 1, lr=args.lr)
	...
	for epoch in tqdm(range(args.epochs)):
		...
		dev_acc, dev_f1, y_pred, y_true,_ = model_eval(dev_dataloader, model, device)
       
		if dev_acc > best_dev_acc:
			best_dev_acc = dev_acc
			save_model(model, optimizer, args, config,args.filepath,None)
...
def generate_sentence(args, prefix, outfile, max_new_tokens = 75, temperature = 0.0):
	with torch.no_grad():
		...
		llama,_ = load_pretrained(args.pretrained_model_path)
...
def test(args):
	...
	with torch.no_grad():
		...
		model = LlamaEmbeddingClassifier(config)
		# Adding lora specific configuration
		if args.option == 'lora':
		...
		test_acc, test_f1, test_pred, test_true, test_sents = model_eval(test_dataloader, model, device)
		if args.error_dev:
		    print_wrong_sentences(dev_pred,dev_true,dev_sents,args.error_dev)
		if args.error_test:
		    print_wrong_sentences(test_pred,test_true,test_sents,args.error_test)
		write_predictions_to_file("dev", args.dev_out, dev_acc, dev_pred, dev_sents)
		write_predictions_to_file("test", args.test_out, test_acc, test_pred, test_sents)
...
def get_args():
	...
	parser.add_argument("--pretrained_model_path", type=str, default="stories42M.pt")
	parser.add_argument("--max_sentence_len", type=int, default=None)
	parser.add_argument("--seed", type=int, default=1337)
	parser.add_argument("--epochs", type=int, default=5)
	parser.add_argument("--option", type=str,
						help='prompt: the Llama parameters are frozen finetune: Llama parameters are updated;lora:Low rank adaptation for fine-tuning;pretrain:Continue pretraining with new data',
						choices=('generate', 'prompt', 'finetune','lora','pretrain'), default="generate")
	parser.add_argument("--rank", help = 'specify rank for lora fine-tuning. The default value is 4',type=int,default=4)
	parser.add_argument("--alpha", help = 'specify alpha for lora fine-tuning. The default value is 1',type=int,default=1)
	parser.add_argument("--lora_dropout", help = 'specify dropout for lora fine-tuning. The default value is 0',type=float,default=0)
	parser.add_argument("--modules", help = 'Specify target modules as comma separated strings',default = "compute_query,compute_value")
	...
	parser.add_argument("--error_dev",type = str,default = None)
	parser.add_argument("--error_test",type = str,default = None)
	# hyper parameters
...
if __name__ == "__main__":
	args = get_args()
	print('path',args.pretrained_model_path)
	...
	if args.option == "ar":
	    ...
	    '''
	    args.lora_config = LoraConfig(r=8,lora_alpha=16, target_modules=["compute_query", "compute_value"], 
	    ...
	    #Continue pretraining with the new set
	    pretrain(args)
	else:
