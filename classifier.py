...
class LlamaZeroShotClassifier(torch.nn.Module):
	def __init__(self, config: LlamaConfig, tokenizer: Tokenizer, label_names: list[str]):
		...
		self.llama,_ = load_pretrained(config.pretrained_model_path)
		...
		self.label_name_ids = [tokenizer.encode(label, bos=False, eos=False,show=True) for label in label_names]
...
class LlamaEmbeddingClassifier(torch.nn.Module):
	def __init__(self, config):
		...
		for param in self.llama.parameters():
			if config.option == 'pretrain' or config.option == 'lora':
				param.requires_grad = False
