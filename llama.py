...
class RMSNorm(torch.nn.Module):
    ...
    def _norm(self, x):
        ...
        """
        norm_val = torch.sqrt(torch.mean(x ** 2, dim=-1, keepdim=True) + self.eps)
        return x / norm_val
...
class Attention(nn.Module):
    ...
    def compute_query_key_value_scores(self,
        ...
        attention_scores = self.attn_dropout(F.softmax(scores,dim =-1))
       
        value_vectors = torch.matmul(attention_scores,value)
...
class LlamaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_out += x
        normalized_out = self.ffn_norm(attn_out)
        normalized_out = self.feed_forward(normalized_out)
        out = normalized_out + attn_out
        return out
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0):
        ...
        for _ in range(max_new_tokens):
            ...
            if temperature == 0.0:
                ...
                idx_next = logits.argmax(dim = -1,keepdims=True)
            else:
                '''
                ...
                #top_k_vals,top_k_inds = torch.topk(logits,k=10,dim=-1)
                scaled_logits = F.softmax(logits,dim = -1)
                idx_next = torch.multinomial(scaled_logits,num_samples=1, replacement=True)
                #idx_next = torch.gather(top_k_inds,dim=-1,index = idx_vals)
...
def load_pretrained(checkpoint):
  ...
  #print('checkpoint',checkpoint_dict.keys())
  old_dict = torch.load('stories42M.pt',map_location =device)
  config = LlamaConfig(**old_dict['model_args'])
